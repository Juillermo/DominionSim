...
public class DomBoard extends EnumMap<DomCardName, ArrayList<DomCard>> {
	...
	private EnumMap<DomCardName, ArrayList<DomCard>> separatePiles = new EnumMap<DomCardName, ArrayList<DomCard>>(
			DomCardName.class);
	...
	private void addCustomKingdoms() {
		for (DomPlayer thePlayer : players) {
			...
			for (DomCardName theCard : theNeedsPerPlayer) {
				...
				if (theCard == DomCardName.Marauder || theCard == DomCardName.Bandit_Camp
						|| theCard == DomCardName.Pillage)
	...
	public DomCard take(DomCardName aCardName) {
		...
		gainsNeededToEndGame = 0;
		if (aCardName == DomCardName.Gladiator || aCardName == DomCardName.Fortune || aCardName == DomCardName.Settlers
				|| aCardName == DomCardName.Bustling_Village || aCardName == DomCardName.Catapult
				|| aCardName == DomCardName.Rocks || aCardName == DomCardName.Patrician
				|| aCardName == DomCardName.Emporium || aCardName == DomCardName.Encampment
	...
	public int countEmptyPiles() {
		int theEmptyPiles = 0;
		for (DomCardName theCardName : keySet()) {
			if (!theCardName.hasCardType(DomCardType.Event) && !theCardName.hasCardType(DomCardType.Heirloom)
					&& !theCardName.hasCardType(DomCardType.Landmark))
	...
	public int count(DomCardName aCardName) {
		...
		if (theList == null || theList.isEmpty())
			return 0;
		// if (theList.get(0).getName()!=theCardName &&
		// aCardName.hasCardType(DomCardType.Split_Pile) )
		// return 0;
		return theList.size();
	}
	...
	public void addVPon(DomCardName cardName) {
		...
		if (DomEngine.haveToLog)
			DomEngine.addToLog(cardName + " has " + gatheringVPTokens.get(cardName) + "&#x25BC;");
	}
