...
public class DomDeck extends EnumMap<DomCardName, ArrayList<DomCard>> {
	...
	public DomDeck(DomDeck source, DomPlayer sourcePlayer) {
		...
		while (enumKeySet.hasNext()) {
			...
			for (DomCard item : sourceCards)
				newCards.add(new DomCard(item, null));

			super.put(currentState, newCards);
		}
		for (DomCard item : source.drawDeck)
			drawDeck.add(new DomCard(item, sourcePlayer));
		for (DomCard item : source.discardPile)
			discardPile.add(new DomCard(item, sourcePlayer));
		for (DomCard item : source.islandMat)
			islandMat.add(new DomCard(item, sourcePlayer));
		owner = sourcePlayer;
		for (DomCard item : source.putAsideCards)
			putAsideCards.add(new DomCard(item, sourcePlayer));
	}
	...
	public double getTotalTreasure() {
		...
		for (DomCardName theCardName : keySet()) {
			theTotalMoney += theCardName.hasCardType(DomCardType.Treasure)
					? theCardName.getCoinValue() * get(theCardName).size()
					: 0;
		}
	...
	public void gain(DomCard aCard, int aLocation) {
				...
				if (!owner.getCardsFromPlay(DomCardName.Royal_Seal).isEmpty()) {
					if (owner.isHumanOrPossessedByHuman()) {
						if (owner.getEngine().getGameFrame().askPlayer(
								"<html>On Top of Deck: " + aCard.getName().toHTML() + "</html>",
								"Resolving Royal Seal"))
							owner.putOnTopOfDeck(aCard);
						else
					...
					if (owner.isTravellingFairActive()) {
						if (owner.isHumanOrPossessedByHuman()) {
							if (owner.getEngine().getGameFrame().askPlayer(
									"<html>On Top of Deck: " + aCard.getName().toHTML() + "</html>",
									"Resolving Travelling Fair"))
								owner.putOnTopOfDeck(aCard);
							else
								discardPile.add(0, aCard);
						} else {
							if (aCard.getDiscardPriority(1) > DomCardName.Copper.getDiscardPriority(1)
									|| (aCard.getName() == DomCardName.Copper && owner.getDrawDeckSize() < 8))
								owner.putOnTopOfDeck(aCard);
							else
								discardPile.add(0, aCard);
						}
		...
		if (aCard.hasCardType(DomCardType.Victory)) {
			if (owner.isCardInPlay(DomCardName.Groundskeeper)) {
				for (DomCard theKeeper : owner.getCardsFromPlay(DomCardName.Groundskeeper)) {
					if (DomEngine.haveToLog)
						DomEngine.addToLog(owner + " resolves " + theKeeper);
					owner.addVP(1);
				}
			}
			if (owner.getCurrentGame().getBoard().isLandmarkActive(DomCardName.Aqueduct)) {
				int theVP = owner.getCurrentGame().getBoard().removeVPFrom(DomCardName.Aqueduct,
						owner.getCurrentGame().getBoard().countVPon(DomCardName.Aqueduct));
				if (theVP > 0) {
					if (DomEngine.haveToLog)
						DomEngine.addToLog(owner + " takes VP from " + DomCardName.Aqueduct.toHTML());
					owner.addVP(theVP);
				}
			}
			if (owner.getCurrentGame().getBoard().isLandmarkActive(DomCardName.Battlefield) && owner.getTurns() > 0) {
				...
				if (theVP > 0) {
					if (DomEngine.haveToLog)
						DomEngine.addToLog(owner + " takes VP from " + DomCardName.Battlefield.toHTML());
					owner.addVP(theVP);
				}
			}
		}
		if (aCard.hasCardType(DomCardType.Action)
				&& owner.getCurrentGame().getBoard().isLandmarkActive(DomCardName.Defiled_Shrine)) {
		...
		if (theDuplicate != null) {
			while (theDuplicate != null && ((DuplicateCard) theDuplicate).wantsToGain(aCard)) {
				...
				if (DomEngine.haveToLog)
					DomEngine.addToLog(owner + " calls " + theDuplicate + " from the tavern mat");
	...
	public void showContents() {
		...
		if (get(DomCardName.Distant_Lands) != null) {
			...
			if (theCount > 0) {
				theMessage.append(", ");
				theMessage.append(get(DomCardName.Distant_Lands).size()).append(" ")
						.append(DomCardName.Distant_Lands.toHTML());
	...
	public void putDeckInDiscard() {
		if (DomEngine.haveToLog)
			DomEngine.addToLog(owner + " discards entire deck");
	...
	public void addPhysicalCardWhenNotGained(DomCard aCard) {
		// cards that were passed are not considered 'gained' so can not be
		// watchtowered...
		if (!containsKey(aCard.getName()))
			put(aCard.getName(), new ArrayList<DomCard>());
		get(aCard.getName()).add(aCard);
	...
	public ArrayList<DomCard> revealUntilThreeOfCardNotNamed(DomCardName card) {
		...
		while (theCount < 3 && theTopCard != null) {
			if (DomEngine.haveToLog)
				DomEngine.addToLog(owner + " reveals " + theTopCard);
